...
export const TOKEN_PREFIX = 'mm_';
const BEARER_RE = /^Bearer\s+(mm_\S+)$/i;

// Cookie session (passkeys via AuthGravity) or an mm_ API token minted for
// the pipeline CLI. mm_ tokens are resolved locally by sha256 hash and are
// NEVER forwarded to procauth.
...
// mm_ token can never list, mint, or revoke tokens.
...
/** Mint a new API token: `mm_` + 192 bits of entropy, base64url. */
