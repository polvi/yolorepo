// In-page passkey ceremonies straight against AuthGravity (happybook
// pattern): the session cookie lands on the registrable domain, so a
// cross-origin fetch with credentials works from gpubnb.<base>. The hosted
// procauth surface stays available as the fallback for account keys and
