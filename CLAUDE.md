...
- `pi-local/` — pi.dev running fully local on the M1 Max: llama.cpp (`llama-server`) serving Qwen3.8-27B UD-Q6_K_XL with full Metal offload, Flash Attention, 64K context on a single slot, prefix-cache reuse, vision via mmproj, and Qwen thinking-mode sampling. Helper scripts (`bin/pi-llama-{up,down,status}`) resolve the GGUF out of the Hugging Face cache; `bin/install.sh` merges the `llama-cpp` provider into `~/.pi/agent/models.json` (`--default` makes it pi's default model, `--launchd` starts the server at login). Q6 chosen over Q4/Q5 on measured throughput; 27.9 GiB resident at 64K against a 54.4 GiB Metal working-set limit. Infra-only, no proc subdomain.
