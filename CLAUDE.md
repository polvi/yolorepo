...
- `tabby/` — Splitwise for Monero: group-trip expense splitting settled in XMR via Cake Wallet `monero:` deep links + QR. Expenses in USD/CAD/TAB (1 TAB = 10 USD), normalized to integer µTAB (100,000/TAB) with per-expense fx snapshots; balances always derived (double-entry); greedy minimal-transfer settlement; idempotent writes (client UUID + INSERT OR IGNORE); payer-marks-paid, no on-chain verification. Single worker (Hono + D1 + vanilla-TS SPA), three.js explainer homepage, passkey auth via procauth. Ledger model-checked in `specs/Ledger.tla`. tabby.proc.io
