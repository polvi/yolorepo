...
- `webmtp/` — research: MTP (Media Transfer Protocol) implemented over WebUSB in the browser. Speaks raw PTP containers (OpenSession, GetStorageIDs, GetObjectHandles, GetObjectInfo) against a USB-attached device from Chrome. Directory listing and byte-exact file transfer in both directions (GetObject, SendObjectInfo/SendObject, DeleteObject) verified on real hardware; TLA+ spec of the session/transaction machine in `specs/`. Local dev via `bun serve.ts` on :8321.
