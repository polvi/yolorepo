...
- `pi-local/` — pi.dev running fully local on the M1 Max: llama.cpp (`llama-server`) with full Metal offload, Flash Attention, 64K context on a single slot, prefix-cache reuse, vision via mmproj, and per-model Qwen sampling. Three models registered (`config/models.json`), one loaded at a time; **default is Qwen3.6-35B-A3B UD-Q4_K_XL**, chosen on measurement, not marketing: 686 t/s prefill and 46.9 t/s generation versus 74/5.9 for the dense Qwen3.8-27B Q6, in 23.1 GiB versus 29.1, at equal correctness. Qwen3-Coder-Next 80B-A3B was tested and rejected (bigger, slower, and the only model to fail the hard reasoning task; it is non-reasoning and also under-calls the write tool unless the prompt names it). `bench/eval.ts` is the six-task verified suite behind those claims (filesystem/hidden-test graded; hidden suites validated against known-good and known-bad references) — see `MODELS.md`. Helper scripts `bin/pi-llama-{up,down,status,fetch}`; `pi-llama-fetch` pulls GGUFs over 8 parallel byte ranges because HF throttles a single connection to ~3 MB/s (40 MB/s measured). `bin/install.sh` merges the provider into `~/.pi/agent/models.json` (`--default` sets pi's default model, `--launchd` starts the server at login). Infra-only, no proc subdomain.
