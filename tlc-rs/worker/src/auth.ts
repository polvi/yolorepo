...
// authgravity.proc.io — same registrable domain, so its session_id cookie is
// first-party here). We store only the stable user UUID it returns, plus
// SHA-256 hashes of API keys we mint for MCP bearer auth.

export const AUTHGRAVITY = "https://authgravity.proc.io";
 ...
 */
export async function whoami(request: Request): Promise<string | null> {
