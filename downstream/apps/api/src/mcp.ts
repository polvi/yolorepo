// Stateless MCP endpoint, spec 2026-07-28: no handshake, every request
// self-describing via _meta, Mcp-Method/Mcp-Name headers verified against
// the body (HeaderMismatch -32020). Legacy initialize-era clients (2025-*)
// are still served per the spec's backward-compatibility window.
...
const SERVER_INFO = { name: "downstream", version: "0.1.0" };
...
const rpcError = (id: any, code: number, message: string, status: number, data?: unknown) =>
  Response.json({ jsonrpc: "2.0", id: id ?? null, error: { code, message, ...(data !== undefined ? { data } : {}) } }, { status });
...
export async function handleMcp(env: Env, req: Request, user: AuthedUser): Promise<Response> {
  let body: RpcRequest;
  try {
    body = await req.json();
  } catch {
  ...
  switch (body.method) {
    ...
    case "tools/call": {
      ...
      try {
        const r = await callTool(env, user, name, body.params?.arguments);
        return rpcResult(body.id, {
          resultType: "complete",
